...
void HAL_MspInit(void)
{
   /* Enable PLL, use HSI (= 8MHz) as its time source, subsequently configure
      AHB and APB prescalers in order to generate a 16MHz system frequency */
   ...
   Osc_Handler.PLL.PLLMUL     = RCC_PLL_MUL2;                    /* Multiply by 2, this results in a PLLCLK output = 16MHz */
   ...
   Clk_Handler.SYSCLKSource   = RCC_SYSCLKSOURCE_PLLCLK;         /* Use PLLCLK (16MHz) as source clock for SYSCLK */
   Clk_Handler.AHBCLKDivider  = RCC_SYSCLK_DIV1;                 /* Do not divide SYSCLK, therefore HCLK = SYSCLK (=16MHz) / 1 = 16MHz */
   Clk_Handler.APB1CLKDivider = RCC_HCLK_DIV1;                   /* Do not divide HCLK, therefore PCLK = HCLK (=16MHz) / 1 = 16MHz */
   HAL_RCC_ClockConfig( &Clk_Handler, FLASH_LATENCY_0 );

   /* NOTE: when HAL_RCC_ClockConfig() is called, it also automatically adjusts the
            SysTick reload value for the 1ms timeout occurs needed by the HAL libraries */
}
